...
- Application Map objects are selectable. The inspector shows their parent, children, cardinality, and primary pages, with direct navigation between related objects and pages. Coverage opens from the top navigation, with three warnings in a popover and a full list in a modal.
- Application Network fills the available viewport without vertical canvas scrolling. Pages and objects permits scrolling.
- Application source actions and coverage warnings live in the top navigation. Coverage opens a preview and a full warning dialog.
- Interface labels describe user actions and content. Omit internal terms such as source-backed and canonical.
...
Wireframes compress proposed interface ideas into inspectable diagrams. Named renderer-owned themes (`default` and `recipe`) keep the DSL free of CSS. The language has panels, cards as leaf elements, explicit select, toggle, and checkbox controls, and finite button icon, variant, tone, and state options. Buttons default to `secondary`; `primary` and `quiet` express action hierarchy, while destructive tone remains independent and takes precedence over variant color. General grids use `columns=N min=PX` with a minimum of 120 pixels; calendar layouts use that grid instead of a calendar primitive. Quoted wireframe copy supports explicit `\n` line breaks while retaining automatic wrapping. Cards with `goto` remain keyboard and pointer controls; cards without `goto` render as static content. Textareas and lists support plain, ordered, and checkable modes, with checked items and optional remove actions. Authored navigation and native popover triggers work in the viewer. A compact per-screen state selector shows rest, hover, open, and bounded value/state shots; shot overlays target existing IDs and never mutate the base screen. The removed large progressive-state gallery is not part of the viewer.

...
export function WireframeWorkbench(props: { document: WireframeDocument }) {
  ...
  return <main class="wireframe-app" classList={{ "wireframe-theme-default": props.document.theme === "default", "wireframe-theme-recipe": props.document.theme === "recipe" }}>
    <header class="wireframe-toolbar"><span class="wireframe-source-status">Source-backed wireframe</span><nav aria-label="Wireframe screens"><For each={props.document.screens}>{item => <button type="button" aria-pressed={item.id === screen()?.id} onClick={() => navigate(item.id)}>{item.title}</button>}</For></nav></header>

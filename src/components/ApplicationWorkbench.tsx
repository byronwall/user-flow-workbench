...
function ApplicationWarningList(props: { warnings: readonly ApplicationWarning[] }) {
  return <ul><For each={props.warnings}>{(warning) => <li><strong>{warning.message}</strong><Show when={warning.suggestion}><span>{warning.suggestion}</span></Show></li>}</For></ul>;
}
  ...
  return (
    <div class="app application-app" classList={{ "application-network-view": mapView() === "network" }}>
      <header class="topbar application-topbar">
        <span class="application-source-status" role="status">{stale() ? "Showing last valid application map · source stale" : "Source-backed application map"}</span>
        <div class="spacer" />
        <button class="btn primary" type="button" onClick={() => void refreshSource()}>Reload source</button>
        <button class="btn" type="button" disabled={!sourceText()} onClick={() => downloadSource(currentDocument(), sourceText())}>Export source</button>
      </header>
      <main class="application-main"><div class="application-content">
        ...
        <div class="application-heading"><div><div class="application-title-row"><h1>{currentDocument().title}</h1><span class="application-status-pill">Application map</span></div><p>{currentDocument().purpose || "A source-backed map of pages, objects, and navigation."}</p></div><span class="application-board-note">Explicit page structure · no inferred links</span></div>
        ...
        <Show when={mapView() === "pages"}><section class="application-object-rail" aria-labelledby="application-object-rail-title"><div class="application-section-heading"><div><h2 id="application-object-rail-title">Objects</h2><p>Conceptual records owned by the application or another object.</p></div><span>{currentDocument().objects.length} objects</span></div><div class="application-object-list"><For each={currentDocument().objects}>{(object) => { const owned = currentDocument().ownership.filter((relation) => relation.objectId === object.id); return <article class="application-object-card" classList={{ selected: selectedObjectId() === object.id }} role="button" tabIndex={0} aria-pressed={selectedObjectId() === object.id} onClick={() => selectObject(object.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); selectObject(object.id); } }}><h3>{object.title}</h3><Show when={object.detail}><p>{object.detail}</p></Show><Show when={owned.length} fallback={<span class="application-object-owner">No owner relation authored</span>}><For each={owned}>{(relation) => <span class="application-object-owner">Owned by {relation.ownerId === currentDocument().id ? currentDocument().title : currentDocument().objects.find((item) => item.id === relation.ownerId)?.title || relation.ownerId} · {relation.cardinality}</span>}</For></Show></article> }}</For></div></section></Show>
        <Show when={selectionNotice()}><p class="application-footnote" role="status">{selectionNotice()}</p></Show><p class="application-footnote">Select a page or object to inspect its details.</p>
        <Show when={coverageWarnings().length}><section class="application-coverage" aria-labelledby="application-coverage-title"><div class="application-section-heading"><div><h2 id="application-coverage-title">Coverage warnings</h2><p>Only explicit application links are checked.</p></div><span>{coverageWarnings().length} {coverageWarnings().length === 1 ? "warning" : "warnings"}</span></div><ApplicationWarningList warnings={coverageWarnings().slice(0, 5)} /><Show when={coverageWarnings().length > 5}><details class="application-coverage-more"><summary><span class="application-coverage-summary-show">Show {coverageWarnings().length - 5} more {coverageWarnings().length - 5 === 1 ? "warning" : "warnings"}</span><span class="application-coverage-summary-hide">Hide {coverageWarnings().length - 5} additional {coverageWarnings().length - 5 === 1 ? "warning" : "warnings"}</span></summary><ApplicationWarningList warnings={coverageWarnings().slice(5)} /></details></Show></section></Show>
      </div></main>
      <aside class="sidebar application-sidebar" aria-label="Application details and source"><div class="sidebar-tabs" role="tablist" aria-label="Application sidebar views"><button class="sidebar-tab" id="application-inspector-tab" type="button" role="tab" aria-controls="application-inspector-panel" aria-selected={activeTab() === "inspector"} tabIndex={activeTab() === "inspector" ? 0 : -1} onClick={() => setActiveTab("inspector")} onKeyDown={handleTabKeyDown}>Inspector</button><button class="sidebar-tab" id="application-source-tab" type="button" role="tab" aria-controls="application-source-panel" aria-selected={activeTab() === "source"} tabIndex={activeTab() === "source" ? 0 : -1} onClick={() => setActiveTab("source")} onKeyDown={handleTabKeyDown}><span>Diagram DSL</span><small>Read only</small></button></div><div class="application-sidebar-content"><section class="sidebar-panel application-sidebar-panel" id="application-inspector-panel" role="tabpanel" aria-labelledby="application-inspector-tab" hidden={activeTab() !== "inspector"}><Show when={selectedObject()} fallback={<PageInspector document={currentDocument()} page={selectedPage()} selectedStateId={selectedStateId()} documentPath={props.documentPath} referenceWarnings={referenceWarnings()} onSelectState={selectState} onSelectObject={selectObject} />}>{(object) => <ObjectInspector document={currentDocument()} object={object()} onSelectObject={selectObject} onSelectPage={selectPage} />}</Show></section><section class="sidebar-panel application-sidebar-panel" id="application-source-panel" role="tabpanel" aria-labelledby="application-source-tab" hidden={activeTab() !== "source"}><div class="application-source-body"><p class="application-panel-kicker">Read only</p><h2>Diagram DSL</h2><p class="application-muted">Current canonical source from the selected application map.</p><pre class="application-source-code"><code>{sourceText() || "Source text is unavailable for this document."}</code></pre><p class="application-source-note">The agent owns source edits. Reload the diagram to inspect changes.</p></div></section></div></aside>
    </div>
  );
}

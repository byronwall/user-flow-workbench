...
export interface OverviewFlowInventoryItem {
  path: string;
  title: string;
  linkedFrom: string[];
  relationship?: { capabilityId: string; variant?: string };
}
...
export function overviewFlowInventory(
  entries: readonly DiagramCatalogEntry[],
  overviewPath: string | undefined,
  view: OverviewDocument,
): OverviewFlowInventoryItem[] {
  const folder = directoryOf(overviewPath);
  const relationships = new Map<string, { capabilityId: string; variant?: string; titles: Set<string> }>();
  for (const capability of allCapabilities(view)) {
    for (const reference of capability.flowRefs || []) {
      ...
      else relationships.set(reference.path, { capabilityId: capability.id, ...(reference.variant ? { variant: reference.variant } : {}), titles: new Set([capability.title]) });
    }
  }

  const flows = new Map<string, DiagramCatalogEntry>();
  for (const entry of entries) {
    if (entry.valid && entry.type === "flow" && !flows.has(entry.path)) flows.set(entry.path, entry);
  }
  return [...flows.values()]
    ...
    .map((entry) => {
      ...
      return {
        ...
        ...(relation ? { relationship: { capabilityId: relation.capabilityId, ...(relation.variant ? { variant: relation.variant } : {}) } } : {}),
      };
    })
